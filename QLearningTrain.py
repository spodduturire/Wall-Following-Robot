...
# We select 5 zones from our LiDar and split into different states based on distance
...
# There are total 7 different combinations for a single zones.
def get_state():
    global zones
...
def main():
    ...
    # We have a set of location from which we want our robot to spawn and then start training. This is because our robot has only right side
    # sensors. So we are spawning it such that it always spawns with a wall on it's right side and then starts training.
    ...
    while True:
        ...
        while True:
            ...
            #QLearning Update Step
            q_table[current_state_index][current_action_index]  = q_table[current_state_index][current_action_index] + alpha * (reward + (gamma * max(q_table[next_state_index][:])) - q_table[current_state_index][current_action_index])
